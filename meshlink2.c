...
#define NLATMAX 1500      /* maximum lattice dimension */
	       ...
	       int **meshparts,int ****meshstart,int meshfac)
{
   ...
   fprintf(stderr,"Making mesh with n=%d (size= %.1f MB) %e %e %e\n",
   	   nlatt,sizeof(int)*nlatt*nlatt*nlatt/(1024.*1024.),smin,smax,rmax);
   
   /*
   *meshstart=ALLOC3D(int,nlatt,nlatt,nlatt);
   */
   *meshstart=(int ***)i3tensor(0,nlatt-1,0,nlatt-1,0,nlatt-1);
